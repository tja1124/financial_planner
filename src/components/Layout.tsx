...
export function Layout({
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50">
      <header className="bg-white/95 backdrop-blur-md border-b border-slate-200/90 sticky top-0 z-30">
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <div className="h-14 flex items-center justify-between gap-3">
            ...
            >
              <span className="flex items-center justify-center w-8 h-8 rounded-lg bg-indigo-600 text-white text-sm font-bold group-hover:bg-indigo-700 transition-colors">
                FP
              </span>
      ...
      {lastSaved && (
        <p className="hidden lg:block fixed bottom-3 left-1/2 -translate-x-1/2 text-[10px] text-slate-400 z-10 pointer-events-none">
          Saved locally · {lastSaved}
        </p>
      )}

      <nav
        className="lg:hidden fixed bottom-0 left-0 right-0 bg-white/98 backdrop-blur-md border-t border-slate-200 z-30 safe-area-pb shadow-[0_-4px_20px_rgba(0,0,0,0.06)]"
